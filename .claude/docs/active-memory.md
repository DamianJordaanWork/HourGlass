...
## Current phase: **F3 (web SQLite) done — sql.js WASM SQLite is now the web persistence backend, with a localStorage fallback + one-time import. Next: F4 (Tauri-sql driver for desktop, reusing `createSqlRepositories` unchanged).**
